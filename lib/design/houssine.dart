...
class _houssinePageState extends State<houssinePage> {
  ...
  Future<void> _getUsers() async {
    final response = await http.get(Uri.parse('http://192.168.239.126/flutter-login-signup/history.php'));
  ...
  Future<void> _deleteUser(User user) async {
    final response = await http.post(
      Uri.parse('http://192.168.239.126/flutter-login-signup/deletehoussine.php'),
      body: {'id': user.id},
    );
    if (response.statusCode == 200) {
      setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      drawer: NavBar(),
      appBar: AppBar(
        title: Text('Historique d\'accès de Houssine'),
