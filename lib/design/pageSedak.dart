...
class SedakPage extends StatefulWidget {
  const SedakPage({Key? key}) : super(key: key);

  @override
  State<SedakPage> createState() => _SedakPageState();
}

class _SedakPageState extends State<SedakPage> {
  List<User> _users = [];
  ...
  Future<void> _getUsers() async {
    final response = await http.get(Uri.parse('http://192.168.239.126/flutter-login-signup/sedak.php?name=sedak'));
  ...
  Future<void> _deleteUser(User user) async {
    final response = await http.post(
      Uri.parse('http://192.168.239.126/flutter-login-signup/deleteSedak.php'),
      body: {'id': user.id},
    );
    if (response.statusCode == 200) {
      setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      drawer: NavBarutil(),
      appBar: AppBar(
        title: Text('Historique d\'accès de Sedak'),
